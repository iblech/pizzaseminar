...
nicht immer so ist, zeigt schon das Beispiel~$H^0(\AA^1, \O_{\AA^1})$. Dieser
Vektorraum ist der unendlichdimensionale Raum der holomorphen Funktionen auf~$\CC$. Hier und im
...
die exakte Sequenz~$0 \to (z) \to \O_{\AA^1} \to i_*\O_\pt \to 0$. Dabei ist~$(z)$ die
...
abgeschlossener Unterschemata gehört die Idealgarbe~$\J + \J'$. Die vorgedrückte
...
regulär, und in der Tat hat das Verschwindungsschema~$V(z,z) = V(z)$ dieser Funktionen
...
\begin{ex}Sei~$X = \PP^1$ mit den homogenen Koordinaten~$x$ und~$y$ und sei~$H = V(x)$.
...
frei ist. Dazu benötigen wir folgendes Faktum:\footnote{Dieses Faktum drückt die
...
auf~$X$ modulo Isomorphie, versehen mit dem Tensorprodukt als Gruppenverknüpfung,
...
\emph{unter einer lokalen Trivialisierung} definiert und ist nicht stets
trivial.

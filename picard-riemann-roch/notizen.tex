...
\[ (C \cdot C') := \chi(\O_C \otimes_{\O_X}^{\mathbb{L}} \O_{C'}) :=
  \sum_{n = 0}^\infty (-1)^n \, \chi(\mathcal{T}or^{\O_X}_n(\O_C,\O_{C'})) \in \ZZ. \]
\end{defn}
...
\begin{ex}XXX Selbstschnitt allgemein
\end{ex}

\begin{ex}XXX Aufblasung eines Punkts in der projektiven Ebene
\end{ex}

\begin{rem}Die Schnittzahl lässt sich auch rein topologisch, ohne Rückgriff auf
die algebraische bzw. holomorphe Struktur, beschreiben. Dazu sei an ein Faktum
über die erste Chernklasse erinnert: Ist~$i : C \hookrightarrow X$ die
...
XXX). Daher folgt für Kurven~$C$ und~$C'$ auf~$X$
\begin{align*}
  (C \cdot C')_{\text{top.}}
  &:= \int_X \PD(C) \cup \PD(C')
  = \int_X c_1(\O(C)) \cup c_1(\O(C')) \\
  &= \int_C i^*(c_1(\O(C')))
  = \int_C c_1(i^*\O(C'))
  = \int_C c_1(\O(C')|_C) \\
  &= \deg \O(C')|_C

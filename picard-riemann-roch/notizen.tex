...
in lokalen Systemen benötigt. XXX Quelle}
...
Dann besitzt~$\O(H)$ den globalen Schnitt~$1$. Die Garbe~$\O(-H)$ hat nur den
...
einpunktigen Unterschema in~$\PP^1$ ist durch Serres Twistingbündel~$\O(-1)$ gegeben. Die Idealgarbe
einer Hyperfläche, welche durch eine homogene Gleichung vom Grad~$d$
ausgeschnitten wird, ist~$\O(-d)$.
\end{ex}


\subsection*{Divisoren auf~$\PP^1$}
...
nullte Kohomologie vorkommt. (XXX Genauer)

XXX Geschlecht und~$\chi(\O)$
...
gewöhnliche Tensorprodukt genügt, die höheren Tor-Terme also verschwinden.\footnote{XXX Erklärung} In
diesem Fall gilt
...
\begin{prop}Sei~$C$ in~$X$ regulär eingebettet. Sei~$\N_{C/X}^\vee \cong \O(-C)|_C$ das
Konormalenbündel und~$\O(C)|_C$ das Normalenbündel. Dann ist
...
höhere Dimension hat -- nur muss~$C$ dann von halber Dimension sein und man
...
wir diesen Isomorphismus auch auf globaler Ebene. XXX Quelle
\end{proof}
...
\begin{fact}Sei~$\omega := \Lambda^2 \Omega^1_X$ das Bündel der
Differentialformen im Topgrad. Dann ist~$H^2(X,\omega)$ eindimensional und
für~$i = 0,1,2$ induziert das Cup-Produkt für jedes Geradenbündel~$\L$ eine
nicht ausgeartete Bilinearform.
\[ H^i(X,\L) \times H^{2-i}(X,\omega \otimes \L^\vee) \lra
  H^2(X,\omega) \stackrel{\cong}{\lra} \CC \]
\end{fact}

XXX Bedeutung, Intuition, Poincaré-Dualität, \ldots

\begin{cor}Für jedes Geradenbündel~$\L$ gilt~$\chi(\L) = \chi(\omega \otimes
\L^\vee)$.\end{cor}
\begin{proof}Es gilt~$\dim H^i(X,\L) = \dim H^{2-i}(X,\omega \otimes \L^\vee)$,
und die Vorzeichen passen.
\end{proof}

\begin{thm}[Riemann--Roch für Flächen]
Sei~$\L$ ein Geradenbündel auf~$X$. Dann gilt
\[ \chi(\L) = \chi(\O_X) + \tfrac{1}{2} \cdot \bigl((\L \cdot \L) - (\L \cdot \omega)\bigr). \]
\end{thm}

\begin{proof}Die Behauptung folgt durch Umstellen der Gleichung
\begin{align*}
  -(\L \cdot \L) + (\L \cdot \omega)
  &= (\L^\vee \cdot \L) + (\L^\vee \cdot \omega^\vee) \\
  &= (\L^\vee \cdot (\L \otimes \omega^\vee)) \\
...
Es gilt~$\chi(\O_X) = \frac{1}{12} \cdot ((\omega \cdot \omega) +
\chi_\mathrm{top}(X))$.
\end{fact}
...
Sei~$C$ eine irreduzible Kurve auf~$X$. Für ihr \emph{Geschlecht} $g(C) :=
\dim_\CC H^1(C,\O_C)$ gilt
\[ g(C) = 1 + \frac{1}{2} \cdot ((C \cdot C) + (C \cdot K)). \]
\end{cor}

XXX Adjunktionsformel

XXX Werbung für Shafarevich und Manin; für Divisoren auch Huybrechts

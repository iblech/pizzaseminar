...
Genau dann ist ein Ringelement~$f \in R$ invertierbar, wenn
$R \models \exists g\?\O\_ fg = 1$.\end{prop}
\begin{proof}
...
Diese beiden Propositionen war noch nicht besonders beeindruckend.
...
einer beliebig kleinen Umgebung von~$(a,b) \in K^2$ definierten rationalen
...
Zerlegung der Eins gibt, sodass sie in den jeweils lokalisierten Ringen gilt.
...
mit Ringen zu arbeiten, ohne manuell Zerlegungen einzuführen und mitzuschleppen
zu müssen.
...
\emph{mathematisch argumentieren} -- fast genau so, wie wir es gewöhnt sind.
...
\begin{proof}Ein Gegenbeispiel liefert der Ring~$R := \ZZ$ und das Element~$f
:= 2$. Denn
\[ R \models \speak{$f$ inv.} \vee \neg(\speak{$f$ inv.}) \]
würde bedeuten, dass es eine Zerlegung~$1 = s_1 + \cdots + s_n$ der Eins

...
% XXX: van Dalen und Locales and toposes as spaces, Vickers, zitieren.
...
% XXX: Jahreszahl
% XXX: Wortlaut der Zitate nachsehen, genaue Quellen angeben
...
% XXX: Hilft universelles Ringelement?
...
% XXX: Referenz?
\end{bem}
% XXX: Kann man das generische Primideal in einem anderen Topos realisieren?
...
Suprema existieren und für diese folgendes Distributivitätsgesetz erfüllt ist:
...
Grundlegende Konzepte können also nicht mehr geometrische über einen Phasenraum
...
\begin{bsp}Jeder reine Zustand ist auch ein gemischter Zustand.\end{bsp}
...
\Omega$ stehen. In der Kategorie der Mengen ist~$\Omega = \P(\{\star\})$ (in
klassischer Logik also~$\Omega = \{ 0, 1 \}$) ein solches: Denn bekanntlich
...
Obige Definition ist völlig korrekt, ist für unsere Zwecke aber aus zwei
...
Archetypisches Beispiel für einen Topos ist die Kategorie der Mengen. Ferner
ist die Kategorie der mengenwertigen Garben über einem topologischen Raum~$X$
ein Topos.

% XXX: Topostheorie wichtig; viele Facetten; Verallgemeinerung von top. Räumen
% und von Örtlichkeiten; Kripke-Rahmen; und siehe Foliensatz...

%~\cite[Seite~209]{scheibe}:
%\begin{quote}
...
% X leer. Dann X subset Y zeigen. Klären, wieso das gute Vorgehen tatsächlich
% gut ist.

% XXX: Formel vs. Aussage

=== Quantenmechanische Systeme als C^*-Algebren
 
Ein quantenmechanisches System wird beschrieben durch eine (im
Allgemeinen nichtkommutative) C^*-Algebra A.

Wichtigstes Beispiel: A = L(H,H) = Raum der linearen stetigen Operatoren
von H nach H, wobei H ein Hilbertraum ist. (H kann etwa der Zustandsraum
eines Systems sein, wie man es wohl in jeder Vorlesung über
Quantenmechanik lernt.)

(Dieses Beispiel kann auch unser einziges Beispiel bleiben. Dann müssen
wir nicht extra C^*-Algebren thematisieren.)

Wörterbuch:

* Observable = selbstadjungierte Elemente a in A
* Zustände: C-lineare Abbildungen p : A --> C (positiv und normalisiert)
* (Erwartungs-)Wert der Observablen a im Zustand p: p(a)
* Ein-Parameter-Untergruppe R --> Aut(A): Evolution des Systems

Das sollte man an einem einfachen Beispiel verdeutlichen. Hierin liegt
vermutlich deine Hauptarbeit.

Nun der erste kritische Punkt: Sei p : A --> C eine beliebige Abbildung
(nicht unbedingt linear). Seien a1 und a2 zwei Observablen, also
zwei selbstadjungierte Elemente in A. Wenn a1 und a2 miteinander
kommutieren, stellt man folgende Forderung: Wenn p wirklich einen
Zustand beschreiben soll, muss p(a1 + a2) = p(a1) + p(a2) gelten. Wenn
a1 und a2 aber nicht miteinander kommutieren, ist diese Forderung
unsinnig.
 
Diesen Absatz muss man quantenphysikalisch-anschaulich erklären.

Davon motiviert trifft man folgende Definition:
Ein *Quasi-Zustand* ist eine Abbildung A --> C, welche nicht
unbedingt linear ist, aber auf allen kommutativen Unteralgebren von A
linear ist. Außerdem sollte sie positiv und normalisiert sein.

In klassischer Mechanik kommutieren je zwei Observablen, daher ist ein
Quasi-Zustand klassisch nichts anderes als ein echter Zustand. In
Quantenmechanik stimmt das aber nicht mehr.

Hier setzt dann der Bohr-Topos an: Er ist ein alternatives
Mathematik-Universum, aus dessen Sicht A kommutativ und daher jeder
Quasi-Zustand schon ein Zustand ist! Das thematisieren wir dann zwei
Wochen später.


=== Bohrs Philosophie

Bohr hatte anscheinend die Philosophie, dass man quantenmechanische
Systeme alleine mit klassischen Messungen verstehen kann. Weißt du etwas
darüber? Relevant ist es insofern, als dass diese Philosophie (bzw.
Bohrs wirkliche Philosophie, nicht meine falsche Wiedergabe) durch den
Bohr-Topos umgesetzt wird.


=== Kochen-Specker
 
Eine *Bewertung* auf A ist eine Funktion lambda : A ---> R, die folgende
zwei Eigenschaften erfüllt:
 
* Für jedes selbstadjungierte Element a ist lambda(a) ein Element des
  Spektrums von A.

* Sind a und b zwei selbstadjungierte Elemente von A, die über eine
  Funktion h miteinander gekoppelt sind -- b = h(a) -- dann soll auch
  lambda(b) = h(lambda(a)) gelten.

  (Sind a1 und a2 zwei miteinander kommutierende selbstadjungierte
  Elemente von A, so folgt daraus insbesondere lambda(a1 + a2) =
  lambda(a1) + lambda(a2) und lambda(a1 * a2) = lambda(a1) * lambda(a2).

Eine Bewertung weist also *allen* Observablen einen konsistenten
Messwert zu. Das Kochen-Specker-Theorem besagt: Falls A = L(H,H) und
dim H > 2, so gibt es keine Bewertungen.
 
Das kann man physikalisch irgendwie so interpretieren, dass in einer
Quantentheorie (unter geeigneten Voraussetzungen) keine verborgenen
...
In zwei Wochen werden wir dann verstehen: Die Aussage des
Kochen-Specker-Theorems ist äquivalent dazu, dass ein gewisser Raum (der
Raum der Bewertungen) keinen Punkt enthält. Er enthält aber sehr wohl
Punkte *aus der internen Sicht des Bohr-Topos*!

...
\newcommand{\listedmotto}[2]{%
  \begin{motto}\label{#1}#2\end{motto}%
  \addcontentsline{lom}{section}{\noindent\textnormal{\textbf{Motto \ref{#1}.} #2}}%
}
...
\tocbasic@listhead{Liste der Mottos}
\@starttoc{lom}
\makeatother
...
\[ \forall n \? \NN\_ \varphi(n) \Rightarrow \psi(n) \]
eine Vorschrift, wie man für jede natürliche Zahl~$n : \NN$ aus einem Beleg
für~$\varphi(n)$ einen Beleg für~$\psi(n)$ erhalten kann. Dies soll tatsächlich nur
...
Stages of Accepting Constructive Mathematics}~\cite{bauer:video} (mit schriftlicher Ausarbeitung in~\cite{bauer:stages}). Das
...
\[ (\exists x\_ \text{der Schlüssel befindet sich an Position~$x$}), \]
...
\listedmotto{int-schw-feiner}{Intuitionistische Logik ist schwächer, aber auch feiner als
klassische Logik.}

Wer konstruktive Mathematik abtun möchte, da ein Lieblingstheorem nicht
intuitionistisch ableitbar ist, kann noch Abschnitt~\ref{bez-klassische-logik}
abwarten, in dem wir verstehen, dass sich klassische Logik in intuitionistische
einbetten lässt.
...
\listedmotto{allesformalisierbar}{Alle Beweise gewöhnlicher Mathematik, die man
...
\listedmotto{allesformalisierbar-zu-optimistisch}{Das optimistische Motto~\ref{allesformalisierbar} stimmt nur in
...
wohldefinierten) Zahl bestimmen.
...
Aussagen:
Wir stellen uns ein besonders kritisches Gegenüber ("`Eve"') vor, das unsere Behauptung
...
\listedmotto{int-dialog}{Eine Aussage~$\varphi$ intuitionistisch zu behaupten, bedeutet, in jedem
Dialog~$\varphi$ belegen zu können.}

% XXX: Das lässt sich formalisieren. Auf Helger Rückert in /Essays on
% Non-Classical Logic/ verweisen.
...
\listedmotto{klass-dialog}{Eine Aussage~$\varphi$ klassisch zu behaupten (also~$\varphi^\circ$
...
\listedmotto{motto-ch}{Eine Aussage~$\varphi$ ist genau dann intuitionistisch ableitbar,
wenn es ein Computerprogramm vom Typ~$\varphi$ gibt.}
...
Peano-Arithmetik geben kann~\cite{smith:tears,gusfield:goldilocks}.
...
\listedmotto{motto:inhaltvonbeweisen}{In einem \emph{Beweis} einer
...
Wählt man daher trickreich~$F \defequiv (\exists z\?\NN\_ \varphi[z/y])$, so folgt die Behauptung.
...
können die Nilpotenzindizes bei schlimmen Ringen oder Polynomen beliebig hoch
...
\ZZ[A_0,\ldots,A_n]/(\ldots)$, wobei bei~"`$\ldots$"' die einzelnen
Koeffizienten des Polynoms~$(\sum_{i=0}^n A_i X^i)^m$ stehen sollen, gibt es das \emph{universelle
...
beliebigen Ring~$R$, das der Gleichung~$f^m = 0$ genügt,
genau einen Ringhomomorphismus~$\varphi : S \to R$
...
universellen Polynoms. Nach der Proposition gibt es nun
Exponenten~$m_0,\ldots,m_n$ mit~$A_i^{m_i} = 0$, die nur von~$n$ und~$m$,
nicht aber von den Werten der~$a_i$ eines solchen
Polynoms~$f$ abhängen können. Es folgt~$a_i^{m_i} = \varphi(A_i)^{m_i} =
\varphi(A_i^{m_i}) = \varphi(0) = 0$.}
...
\listedmotto{gen-verwendung}{Die \emph{generische} Verwendung ideeller Konzepte (Primideale,
maximale Ideale, Bewertungen, \ldots) lässt sich eliminieren.}

Der Unterschied zwischen dem \emph{generischen} Primideal und einem
"`beliebigen, aber festen"' Primideal ist in etwa derselbe wie der zwischen der
formalen Polynomvariable~$X \in \RR[X]$ und einem Platzhalter~$x \in \RR$, der
für jede konkrete reelle Zahl stehen kann.
...
Strukturgarbe~$\O_{\Spec R}$. -- Die vorhergehenden drei Sätze erzählen eine
schöne Geschichte, sind aber wörtlich interpretiert nicht ganz richtig: Man
muss die Aussagen so umformulieren, dass sie sich auf das \emph{Komplement} des
generischen Primideals beziehen, den \emph{generischen Filter}; gewisse
eingebaute Negation verdirbt sonst den Spaß. Details stehen
in~\cite{blechschmidt:internal-methods}.}
\end{bem}
...
% Inspiriert von einem Kommentar von John Baez zu einem Google+-Post von
% sigfpe. https://plus.google.com/+DanPiponi/posts/8UJpFYamSBb
...
Unterobjekte eines Objekts~$X$ in Eins-zu-Eins-Korrespondenz zu Morphismen~$X \to
...
\listedmotto{topos-was-ist}{Ein Topos ist eine Kategorie, die dadurch, dass sie ähnliche
kategorielle Eigenschaften wie die Kategorie der Mengen hat, über eine
\emph{interne Sprache} verfügt.}
...
\emph{interne Örtlichkeit}~$\ul{\Sigma} \defeq \Spec \ul{A}$ des Bohr-Topos ist
...
\listedmotto{bohr-motto}{Nichtkommutative \csalgebren~$A$ besitzen zwar keinen
zugehörigen Phasenraum im üblichen mathematischen Universum, wohl aber im speziell auf sie
zugeschnittenen Topos~$\Bohr(A)$.}
...
Eins-zu-Eins-Kor\-res\-pon\-denz zu 
den internen reinen Zuständen von~$\ul{A}$, bzw. wegen Gelfand-Dualität
äquivalent zu internen Punkten von~$\ul{\Sigma}$ (stetigen
Abbildungen~$\ul{\pt} \to \ul{\Sigma}$). Da es, wir in
Bemerkung~\ref{bem:keine-reinen-quasi-zustaende} schon festgehalten haben, im
...
Elementen aus~$\U$ wieder in~$\U$.\footnote{Wörtlich genommen stimmt das nur
für bewohnte Ketten. Der Fall der leeren Kette ist aber trivial. Wenn man diese
Fallunterscheidung unästhetisch findet, kann man auch~$\sum_i \aa_i$
statt~$\bigcup_i \aa_i$ betrachten.} Damit sind alle Voraussetzungen des Lemmas von
...
% XXX: Super witzige Übungsaufgabe von Martin Escardo einbauen.
...
http://sistemas.fciencias.unam.mx/~lokylog/images/stories/Alexandria/Studies%20in%20Logic%20and%20the%20Foundations%20of%20Mathematics/%5BSLFM%20017%5D%20Intuitionism.%20An%20Introduction%20-%20A.Heyting%20%5BStudies%20in%20Logic%20and%20the%20Foundations%20of%20Mathematics%5D%20(NH%201956)(T).pdf

Zitieren:
The Hilbert-Brouwer controversy resolved?
Foundational Theories of Classical and Constructive Mathematics Jacobi and Kummer’s ideal numbers, Franz Lemmermeyer
Friedman, Classically and intuitionistically provably recursive functions
HoTT
Bauer in computable universe
\cite{feferman:kreisel}
\cite{schuster:ivt}
\cite{frank:ivt}
...
- Wie leitet man die Bornsche Regel im Bohr-Topos-Zugang her?

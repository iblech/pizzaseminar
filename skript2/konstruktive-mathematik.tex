...
An die Notation der Ableitungsregeln muss man sich gewöhnen. Drei
Beispiele seien im Folgenden genauer erklärt.
...
die \emph{Doppelnegationsübersetzung}, die Aussagen derart umformt, dass die
...
\begin{satz}Seien~$\varphi$, $\psi$ beliebige Aussagen in einem Kontext~$\vec x$.
\begin{enumerate}
...
Dass die Zeitsprungmetapher berechtigt ist, wollen wir exemplarisch an einem
Beispiel demonstrieren. Dazu betrachten wir einen Dialog zur
...
\begin{defn}Ein \emph{Ideal} eines Rings~$R$ ist eine Teilmenge~$\aa \subseteq
R$, die
\begin{itemize}
...
\item falls ein Produkt in~$\pp$ enthalten ist, schon ein Faktor in~$\pp$
liegt:
...
Satz, welche als einzige unsicherte Voraussetzung diese Eigenschaft hatten.
...
Produkt von Primidealen schreiben lässt. Mit dieser schwächeren Eigenschaft
...
Buch~\ref{schmidt:zahlentheorie} von Alexander Schmidt empfohlen. Als Vorwissen
setzt es nur Schulkenntnisse voraus.


\subsection{Die Ideale im Ring der ganzen Zahlen und im Polynomring über einen
Körper}

Die Tafeln~\ref{ideale:z} und~\ref{ideale:qx} zeigen die Ideale der Ringe~$\ZZ$
und~$\QQ[X]$. Ergänzt man die aus Platzgründen ausgelassenen Ideale, sind das
sogar vollständige Übersichten über \emph{alle} Ideale der jeweiligen Ringe --
...
die Nilpotenz eines Ringselements nachzuweisen.

\begin{prop}[nur klassisch]Im Schnitt aller Primideale eines Rings liegen nur
...
Zorn erfüllt, womit~$\U$ also ein maximales Element enthält: $\mm$.

Man kann nun nachrechnen, dass~$\mm$ ein Primideal ist. Da~$x \not\in \mm$
(wegen~$x \in S$), ist das ein Widerspruch zur Voraussetzung.
\end{proof}

Dieser Beweis ist an zwei zentralen Stellen inhärent klassisch: Zum einen, weil

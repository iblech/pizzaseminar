...
Allgemeinen ist~$N := p_1 \cdots p_n + 1$ für Primzahlen~$p_i$ nämlich keine weitere
Primzahl -- nur die Primfaktoren von~$N$ werden neue Primzahlen
...
die Aussage
\[ \varphi :\equiv (\forall n\?\NN\_ n = m) \]
...
\begin{align*}
  ...
  (\forall x\?X\_ \varphi)^\circ &:\equiv \optnegneg\forall x\?X\_ \varphi^\circ \\
  (\exists x\?X\_ \varphi)^\circ &:\equiv \neg\neg\exists x\?X\_ \varphi^\circ
\end{align*}
\end{defn}
...
\begin{align*}
  ...
  (\forall x\?X\_ \varphi)^F &:\equiv (\forall x\?X\_ \varphi^F) \optF \\
  (\exists x\?X\_ \varphi)^F &:\equiv (\exists x\?X\_ \varphi^F) \vee F
\end{align*}
...
Wählt man daher trickreich~$F :\equiv (\exists y\?\NN\_ \varphi)$, so folgt die Behauptung.
...
\listedmotto{bohr-motto}{Nichtkommutative \csalgebren~$A$ besitzen zwar keine zugehörige
Ört\-lich\-keit im üblichen mathematischen Universum, wohl aber im speziell auf sie
...
\[ \sqrt{\aa} := \{ x \in R \,|\, \exists n \geq 0\_ x^n \in \aa \} \]

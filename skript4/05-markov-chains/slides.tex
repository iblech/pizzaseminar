...
\author[Kleine Bayessche
AG]{\vspace{-1em}\\\includegraphics[scale=0.3]{markov.png} \\[0.5em] Ingo
Blechschmidt}
...
\note{
  \begin{itemize}
    \item A Markov chain is \emph{irreducible} iff any state can transition into
    any state in a finite number of steps.
    \item A Markov chain is \emph{aperiodic} iff any state can transition into
    itself in a single step.
    \item If~$T_{ij} > 0$ for all $i$ and $j$, the chain is irreducible and
    aperiodic.
    \item Think of the chain describing exports of goods by countries:
    $q_i$ is the wealth of country~$i$ (as a fraction of the global wealth).
...
\note{
  ...
  To sample from~$f$, we have to pick any point in~$\{ (x,y) \,|\, y \leq f(x)
  \}$ at random and look at the point's $x$~coordinate.

  We do this by first picking any point in~$\{ (x,y) \,|\, y \leq Mg(x) \}$ --
  this we can do, since we assume that~$g$ is efficiently sampleable.
  XXX
...
\frame[t]{\frametitle{Image sources}
  \begin{itemize}
    ...
    \item \url{http://shetall.files.wordpress.com/2013/11/snakes_and_ladders1.jpg}
    \item \url{https://www.facebook.com/4funsociety/photos/a.176890522366207.59715.176890359032890/594923673896221/}
  \end{itemize}
}

...
\subsubsection{Partielle Beziehungsinformationen}
...
\begin{itemize}
\item Sei~$X$ eine Menge, also ein Objekt von~$\Set$. Als Menge ohne besondere
...
\item Sei~$G$ eine Gruppe, also ein Objekt von~$\Grp$. In Analogie zum
...
neutrale Element von~$G$ schickt. Diese Information legt~$G$ also überhaupt
noch nicht fest.
...
stehen nämlich in natürlicher Bijektion zu den Gruppenelementen, deren
Ordnung~$n$ teilt.

\item Sei~$X$ ein topologischer Raum oder ein strukturierteres geometrisches
...
Ist etwa~$A = [0,1]$ das reelle Einheitsintervall, so beschreiben Morphismen~$A

...
Typs~$\P(X) \to X$ definieren könnte.\footnote{Falls~$X$ die leere Menge ist,
gibt es auch einfach überhaupt keine Abbildung~$\P(X) \to X$. Es gibt aber auch
dann keine natürliche Transformation~$\P \Rightarrow \Id$, wenn man als
Quellkategorie der beiden Funktoren nicht~$\Set$, sondern ihre Unterkategorie
der bewohnten Mengen verwendet.}
\end{enumerate}
...
% 0-1-Prinzip
% 1. sort . map f = map f . sort
% 2. xs sortiert <==> map (>= x) xs sortiert für alle x
% 3. sort xs sortiert <==> map (>= x) (sort xs) sortiert für alle x
%                     <==> sort (map (>= x) xs) sortiert für alle x

% Die Operation, die einer Cauchyfolge in einem vollständigen metrischen Raum
% ihren Grenzpunkt zuordnet, ist eine natürliche Transformation. (Danke an
% Tim!)

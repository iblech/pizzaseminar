...
quasikompakten und quasiseparierten Morphismen ist wieder
...
\mathrm{Mod}(\O_X)$ besitzt einen Linksadjungierten, den so genannten
...
einen~$A$-Modul, den wir~"`$\E(A)$"' bezeichnen möchten. In der Notation
...
kanonische Abbildung~$\E(A) \to \E(B)$. Insgesamt wird definiert daher die
Zuordnung~$(\Spec A \to X) \mapsto \E(A)$ eine \emph{Prägarbe} auf der
Kategorie~$\mathrm{Aff}/X$ der affinen Schemata über~$X$.

Die Familie dieser Moduln~$\E(A)$ hat zwei Besonderheiten:
\begin{enumerate}
\item Seien Morphismen~$f : \Spec A \to X$ und~$p : \Spec B \to \Spec A$ gegeben. Dann
gibt es einen kanonischen Isomorphismus
\[ \E(A) \otimes_A B \stackrel{\cong}{\lra} \E(B), \]
...
\[ \E(A) \to \prod_i \E(A[f_i^{-1}]) \rightrightarrows \prod_{i,j}
\E(A[f_i^{-1},f_j^{-1}]) \]
ein Differenzkerndiagramm. Man sagt auch, die Zuordnung~$(\Spec A \to X)
\mapsto \E(A)$ sei eine \emph{Zariski-Garbe}.
\end{enumerate}

Man kann sich überlegen, dass für eine Prägarbe auf~$\mathrm{Aff}/X$
Eigenschaft~2 schon aus Eigenschaft~1 folgt. Denn das fragliche Diagramm ist
dann isomorph zu
\[ \E(A) \to \prod_i \E(A)[f_i^{-1}] \rightrightarrows \prod_{i,j}
\E(A)[f_i^{-1},f_j^{-1}], \]
...
definiert ein kohärentes System von Moduln~$(\E(A))_{\Spec A \to X}$, also ein
...
\begin{enumerate}
\item einer Familie von Moduln: für jeden Morphismus~$f : \Spec A \to X$
einen~$A$-Modul~$M_A$, und
\item Isomorphismen: für jeden Morphismus~$f : \Spec A \to X$ und jeden
Morphismus~$p : \Spec B \to \Spec A$ einen Isomorphismus~$M_A \otimes_A B \to
M_B$,
\end{enumerate}
